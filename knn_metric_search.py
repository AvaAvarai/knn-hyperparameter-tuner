...
def run_one_metric_phase(args):
    ...
    Compute nearest neighbors once with max_k, cache indices and distances, then
    evaluate every k in k_list by taking prefixes and applying majority vote + tie-break.
    Returns (list of (k, metric_name, accuracy), elapsed_seconds)."""
    ...
    try:
        ...
        # Single NN search for max_k; cache for all smaller k
...
def main():
    ...
    # Build phases: each phase has (phase_name, k_list, max_k). One NN search per metric per phase.
    if args.k_values:
        k_values = sorted({
    ...
    def run_tasks(phase_name, k_list, max_k):
        # One task per metric: compute NN once with max_k, then evaluate all k via prefixes
        task_args = [
